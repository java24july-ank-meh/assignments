...
/* 3.4*/
/*NOT QUITE SURE HOW I RETURN ALL EMPLOYEES WITHOUT GETTING INTO PACKAGE STUFF
WE HAVENT COVERED AND DECLARING SOME TABLE TYPE AND COPYING FROM GOOGLE*/
CREATE OR REPLACE FUNCTION oldEmployees RETURN TABLE AS oldEmpl TABLE;
BEGIN
    SELECT * INTO oldEmpl FROM EMPLOYEE WHERE BIRTHDATE >= TO_DATE('01-JAN-1968');
    RETURN oldEmpl;
END;
...
CREATE OR REPLACE PROCEDURE getManagers(empid IN INTEGER) AS managerResult TABLE
BEGIN
    SELECT * INTO managerResult FROM EMPLOYEE WHERE EMPLOYEEID =
    (SELECT REPORTSTO FROM EMPLOYEE WHERE EMPLOYEEID = empid);
END;

/*4.3*/

CREATE OR REPLACE PROCEDURE customerCompany(cid NUMBER) RETURN 

blajaaknsdjfkasjkfja
