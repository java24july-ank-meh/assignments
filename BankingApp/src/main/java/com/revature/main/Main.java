...
public class Main {
	public static void main(String[] args) {
		...
		loginPrompt(dao, input);
		input.close();

	}
	
	public static void loginPrompt(BankingDAO dao, Scanner input) {
		String in;
	...
	public static void userPrompt(BankingDAO dao, Scanner input, User currUser) {
		String in;
		do {
			...
			case "add":
				...
				int aamt = Math.abs(Integer.parseInt(input.nextLine()));
				try {
					double aresult = dao.updateAccount(aaccname, aamt, currUser);
					System.out.println("Successfully added $" + (double) aamt + " to account " + aaccname + ". New total is $"+aresult);
				}  catch (OverdraftException e) {
					System.out.println(e.getMessage());
				} catch (SQLException e) {
					System.out.println("Error accessing account. Was it spelled correctly?");
				}
				break;
			case "withdraw":
				...
				System.out.println("How much?");
				int wamt = Math.abs(Integer.parseInt(input.nextLine()));
				try {
					double wresult = dao.updateAccount(waccname, -wamt, currUser);
					System.out.println("Successfully withdrew $" + (double) wamt + " from account " + waccname + ". New total is $"+wresult);
				} catch (OverdraftException e) {
					System.out.println(e.getMessage());
				} catch (SQLException e) {
					System.out.println("Error accessing account. Was it spelled correctly?");
	...
	public static void adminPrompt(BankingDAO dao, Scanner input) {
		String in;
		do {
